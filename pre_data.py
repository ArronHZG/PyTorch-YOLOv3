        ...
        for vbb_file in vbb_files:
            ...
            if annos:
                for filename, anno in sorted(annos.items(), key=lambda x: x[0]):
                    print(filename)
                    if "bbox" in anno:
                        anno2txt(os.path.join(vbb_outputdir, sub_dir), filename, anno)
        #             break
        #     break
        # break
...
def main():
    vbb_inputdir = "/mnt/space-2/DataSet/PedestrianDetection/Caltech行人数据集/annotations"
    vbb_outputdir = "/mnt/space-2/DataSet/PedestrianDetection/Caltech行人数据集/annotations-txt"
